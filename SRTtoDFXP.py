...
files = []
files.extend(glob.glob(os.path.join(path, '*.srt'), recursive=True))
files.extend(glob.glob(os.path.join(path, '*', '*.srt'), recursive=True))
files.extend(glob.glob(os.path.join(path, '*', '*', '*.srt'), recursive=True))
files.extend(glob.glob(os.path.join(path, '*', '*', '*', '*.srt'), recursive=True))
...
for filepath in files:
    ...
    # newFile = open(path + "/" + fileName.replace("srt", "dfxp"), "a", encoding="utf-8-sig")
    newFile = open(os.path.join(*filesArray[0:len(filesArray) - 1]) + "/" + fileName.replace("srt", "dfxp"), "a", encoding="utf-8-sig")
